...
interface DictionaryDao {
    ...
    suspend fun insertWordInfos(infos: List<DictionaryEntity>)
    ...
    @Query("DELETE FROM dictionaryentity WHERE word IN(:words)")
    ...
    @Query("SELECT * FROM dictionaryentity WHERE word LIKE '%' || :word || '%'")
    suspend fun getWordInfos(word: String): List<DictionaryEntity>
}